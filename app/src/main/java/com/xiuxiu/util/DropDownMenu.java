...
public class DropDownMenu extends LinearLayout {
 public String tag="tag1";
    ...
    public void setDropDownMenu(@NonNull List<String> tabTexts, @NonNull List<View> popupViews, @NonNull View contentView) {
       // Log.v(tag,"tabtexts值等于"+tabTexts.size()+"popup值等于"+popupViews.size());
       if (tabTexts.size() != popupViews.size()) {
        ...
        ViewGroup parent1 = (ViewGroup) maskView.getParent();
        if (parent1 != null) {
            parent1.removeView(maskView);
        }
        ...
        ViewGroup parent2 = (ViewGroup) popupMenuViews.getParent();
        if (parent2 != null) {
            parent2.removeView(popupMenuViews);
        }
            containerView.addView(popupMenuViews, 2);
            for (int i = 0; i < popupViews.size(); i++) {
                        popupViews.get(i).setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
                          if(popupViews.get(i).getParent()!=null){
                              ViewGroup p1 = (ViewGroup) popupViews.get(i).getParent();
    ...
    private void addTab(@NonNull List<String> tabTexts, int i) {
        ...
        tab.setPadding(dpTpPx(1), dpTpPx(2), dpTpPx(1), dpTpPx(2));
        //添加点击事件
